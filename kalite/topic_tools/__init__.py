    ...
    if EXERCISES.get(language) is None:
        if settings.DO_NOT_RELOAD_CONTENT_CACHE_AT_STARTUP and not force:
            exercises = softload_json(
                EXERCISES_FILEPATH + "_" + language + ".cache", logger=logging.debug, raises=False)
            if exercises:
                EXERCISES[language] = exercises
                return EXERCISES[language]
        EXERCISES[language] = softload_json(
            EXERCISES_FILEPATH, logger=logging.debug, raises=False)
        ...
        if settings.DO_NOT_RELOAD_CONTENT_CACHE_AT_STARTUP:
            try:
                ...
                logging.warn(
                    "Annotated exercise cache file failed in saving with error {e}".format(e=e))
...
def get_content_cache(force=False, annotate=False, language=None):
    ...
    if CONTENT.get(language) is None:
        ...
        CONTENT[language] = SqliteDict(CONTENT_SQLITEPATH)
        annotate = True

    if annotate:
        if settings.DO_NOT_RELOAD_CONTENT_CACHE_AT_STARTUP and not force:
            # content = softload_json(
                # CONTENT_FILEPATH + "_" + language + ".cache",
                # logger=logging.debug, raises=False)
            content = SqliteDict(CONTENT_SQLITEPATH)
            if content:
