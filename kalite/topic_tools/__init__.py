...
from kalite.topic_tools import jsoncache
...
EXERCISES_INDEX_FILEPATH = os.path.join(
    settings.CHANNEL_DATA_PATH, "exercises_index.json")
ASSESSMENT_ITEMS_FILEPATH = os.path.join(
    settings.CHANNEL_DATA_PATH, "assessmentitems.json")
...
if not os.path.exists(settings.CHANNEL_DATA_PATH):
    logging.warning(
        "Channel {channel} does not exist.".format(channel=settings.CHANNEL))
...
def get_topic_tree(force=False, annotate=False, channel=None, language=None, parent=None):
    ...
    if annotate or TOPICS.get(channel, {}).get(language) is None:
        topics = softload_json(
            TOPICS_FILEPATHS.get(channel), logger=logging.debug, raises=False)
    ...
    if annotate:
        if settings.DO_NOT_RELOAD_CONTENT_CACHE_AT_STARTUP and not force:
            topics = softload_json(TOPICS_FILEPATHS.get(
                channel) + "_" + language + ".cache", logger=logging.debug, raises=False)
            if topics:
        ...
        def recurse_nodes(node, parent=""):
            ...
            # If child_availability is empty then node has no children so we
            ...
            with i18n.translate_block(language):
                ...
                node["description"] = _(node.get("description", "")) if node.get(
                    "description") else ""
        ...
        if settings.DO_NOT_RELOAD_CONTENT_CACHE_AT_STARTUP:
            try:
                ...
                logging.warn(
                    "Annotated topic cache file failed in saving with error {e}".format(e=e))
...
def get_exercise_cache(force=False, language=None):
    ...
    if EXERCISES.get(language) is None:
        ...
        '''
        exercises_index = softload_json(
            EXERCISES_INDEX_FILEPATH, logger=logging.debug, raises=False)
        EXERCISES[language] = jsoncache.jsoncache(10)
        # English-language exercises live in application space, translations in
        # user space
        if language == "en":
            exercise_root = os.path.join(
                settings.KHAN_EXERCISES_DIRPATH, "exercises")
        else:
            exercise_root = os.path.join(settings.USER_DATA_ROOT, "exercises")
        if os.path.exists(exercise_root):
            exercise_path = os.path.join(
                exercise_root, language) if language != "en" else exercise_root
            try:
        ...
        for index in exercises_index:
            exercise = EXERCISES[language][index]
            ...
            if exercise.get("uses_assessment_items", False):
                ...
                exercise_lang = i18n.select_best_available_language(
                    language, available_codes=available_langs)
            ...
            with i18n.translate_block(exercise_lang):
                ...
                exercise["description"] = _(
                    exercise.get("description", "")) if exercise.get("description") else ""
        '''
        if settings.DO_NOT_RELOAD_CONTENT_CACHE_AT_STARTUP:
            try:
...
def get_leafed_topics(force=False, language=None):
    ...
    if LEAFED_TOPICS is None or force:
        ...
        LEAFED_TOPICS = [topic for topic in topic_cache.values() if [child for child in topic.get(
            "children", []) if topic_cache.get(child, {}).get("kind") != "Topic"]]
...
def get_content_cache(force=False, annotate=False, language=None):
    ...
    if CONTENT.get(language) is None:
        CONTENT[language] = softload_json(
            CONTENT_FILEPATH, logger=logging.debug, raises=False)
        annotate = True

    if annotate:
        if settings.DO_NOT_RELOAD_CONTENT_CACHE_AT_STARTUP and not force:
            content = softload_json(
                CONTENT_FILEPATH + "_" + language + ".cache", logger=logging.debug, raises=False)
            if content:
        ...
        if os.path.exists(i18n.get_srt_path()):
            for (dirpath, dirnames, filenames) in os.walk(i18n.get_srt_path()):
                # Only both looking at files that are inside a 'subtitles'
                # directory
        ...
        for content in CONTENT[language].values():
            ...
            if dubmap:
                content_lang = i18n.select_best_available_language(
                    language, available_codes=dubmap.keys()) or ""
                if content_lang:
            ...
            subtitle_lang_codes = subtitle_langs.get(
                "{id}.srt".format(id=content.get("id")), [])

            # Generate subtitle URLs for any subtitles that do exist for this
            # content item
            subtitle_urls = [{
                "code": lc,
            ...
            content["subtitle_urls"] = sorted(
                subtitle_urls, key=lambda x: x.get("code", ""))
            ...
            with i18n.translate_block(content_lang):
                ...
                content["description"] = _(
                    content.get("description")) if content.get("description") else ""
        ...
        if settings.DO_NOT_RELOAD_CONTENT_CACHE_AT_STARTUP:
            try:
                ...
                logging.warn(
                    "Annotated content cache file failed in saving with error {e}".format(e=e))
...
def get_slug2id_map(force=False):
    ...
    if SLUG2ID_MAP is None or force:
        SLUG2ID_MAP = generate_slug_to_video_id_map(
            get_node_cache(force=force))
    return SLUG2ID_MAP
...
def generate_slug_to_video_id_map(node_cache=None):
    ...
    for content_id, c in node_cache.get('Content', {}).iteritems():
        try:
            assert c[
                "slug"] not in slug2id_map, "Make sure there's a 1-to-1 mapping between slug and content_id"
        except AssertionError as e:
            logging.warn(
                _("{slug} duplicated in topic tree - overwritten here.").format(slug=c["slug"]))
...
def get_all_leaves(topic_node=None, leaf_type=None):
    ...
    leaves = [topic for topic in get_topic_tree() if (not leaf_type or topic.get(
        "kind") == leaf_type) and (topic_node.get("path") in topic.get("path"))]
...
def get_exercise_data(request, exercise_id=None):
    exercise = get_exercise_cache(
        language=request.language).get(exercise_id, None)
...
def get_assessment_item_data(request, assessment_item_id=None):
    try:
        assessment_item = main_models.AssessmentItem.objects.get(
    ...
    try:
        item_data = json.loads(
            assessment_item.item_data, object_hook=json_ascii_decoder)
...
def smart_translate_item_data(item_data):
    ...
    if isinstance(item_data, basestring):
        ...
        if 'content' in item_data:
            item_data['content'] = _(
                item_data['content']) if item_data['content'] else ""
...
def get_content_data(request, content_id=None):
    ...
    if not content.get("content_urls", None):
        if request.is_admin:
            # TODO(bcipolli): add a link, with querystring args that
            # auto-checks this content in the topic tree
            messages.warning(request, _(
                "This content was not found! You can download it by going to the Manage > Videos page."))
        elif request.is_logged_in:
            messages.warning(request, _(
                "This content was not found! Please contact your coach or an admin to have it downloaded."))
        elif not request.is_logged_in:
            messages.warning(request, _(
                "This content was not found! You must login as an admin/coach to download the content."))
...
def convert_leaf_url_to_id(leaf_url):
    ...
    assert(
        len(leaf_id) == 1), "Something in leaf ID is malformed: %s" % leaf_url
    return leaf_id[0]
